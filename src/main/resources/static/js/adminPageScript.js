...
function setComandas(comandas) {
	...
	const $table = $("<table class='table' />")
		.append($("<thead />")
			.append($("<tr />")
				.append($("<th />").text("Cliente"))
				.append($("<th />").text("Total"))
				.append($("<th />").text("Dirección"))
				.append($("<th />").text("Comentarios"))
				.append($("<th />").text("Productos"))
				.append($("<th />").text("Estado"))
				.append($("<th />").text("Teléfono"))
				.append($("<th />").text("Modificar"))
				.append($("<th />").text("Borrar"))
			)
		);
	...
	comandas.forEach(function(comanda) {
		...
		//$tableBody.append($("<tr />").addClass(claseEstado)	//para cambair el color dependiendo del estado
		$tableBody.append($("<tr />").attr("id", "comanda-" + comanda.id)
			.append($("<td />").text(comanda.usuario.nombre))
			.append($("<td />").text(comanda.total))
			.append($("<td />").text(comanda.direccionEntrega))
			.append($("<td />").text(comanda.comentarios))
			.append($("<td />").append(comanda.productos.reduce(function(accum, prod) {
				return accum.append($("<li class='listaProductos' />").text(prod.nombre));
			}, $("<ul />"))))
			.append($("<td />").text(estadoATexto(comanda.estado)))
			.append($("<td />").text(comanda.usuario.telefono))
			.append($("<td />").append($btnModificar))
			.append($("<td />").append($btnBorrar))
